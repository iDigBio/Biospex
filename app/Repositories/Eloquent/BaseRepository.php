...
use App\Repositories\Contracts\CacheableContract;
...
abstract class BaseRepository implements RepositoryContract, CacheableContract
{
    ...
    {
        $skipUri = $this->getContainer('config')->get('repository.cache.skip_uri');
    ...
    protected function prepareQuery($model)
    {
        ...
        // Add "has" clause to the quer
        foreach ($this->has as $has)
        {
            list($relation, $operator, $count, $boolean, $callback) = array_pad($has, 5, null);
    ...
    {
        $model = $this->getContainer('config')->get('repository.models');
     ...
     * {@inheritdoc}
     */
    public function store($id, array $attributes = [], $syncRelations = false)
    {
