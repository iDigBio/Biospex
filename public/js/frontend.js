...
var isBuf = __webpack_require__(54);
...
var Backoff = __webpack_require__(127);
...
var JSONP = __webpack_require__(123);
...
Vue.component('passport-clients', __webpack_require__(131));

Vue.component('passport-authorized-clients', __webpack_require__(137));

Vue.component('passport-personal-access-tokens', __webpack_require__(142));
...
/* harmony import */ var __WEBPACK_IMPORTED_MODULE_0_laravel_echo__ = __webpack_require__(104);
...
window.io = __webpack_require__(105);
    ...
    if ($("#chartdiv").length > 0) {
        ...
        var chart = AmCharts.makeChart("chartdiv", {
            ...
            "dataLoader": {
                ...
                "format": "json",
                "showErrors": true,
                "postProcess": function postProcess(data, config, chart) {
                    var graphs = [],
                        hidden_graphs = [],
                        chartData = [],
                        current_day;
...
/* WEBPACK VAR INJECTION */(function($) {$('[data-toggle=confirmation]').confirmation({
...
/* WEBPACK VAR INJECTION */(function($) {$(document).ready(function () {
    ...
    $('#add_target').on('click', function () {
        var first = $('div.target:first');
        var last = $('div.target:last');

        if (first.is(":hidden")) {
            first.show();
        } else {
            last.after(last.clone().find(':input').each(function () {
    ...
    var homeProjectList = $('a.home-project-list');
    ...
    if ($('#processModal').length) {
        Echo.channel(Laravel.ocrChannel).listen('PollOcrEvent', function (e) {
            var uuids = Laravel.groupUuids;
            var ocrHtml = '';
        ...
        Echo.channel(Laravel.exportChannel).listen('PollExportEvent', function (e) {
            var uuids = Laravel.groupUuids;
            var exportHtml = '';
...
$(function () {
    $(document).on('click', '.btn-add', function (e) {
        ...
        controls.find('.entry:not(:last) .btn-add').removeClass('btn-add').addClass('btn-remove').removeClass('btn-success').addClass('btn-danger').html('<span class="fa fa-minus fa-lrg"></span>');
...
/* WEBPACK VAR INJECTION */(function($) {$.jgrid.defaults.width = 780;
$.jgrid.defaults.responsive = true;
$.jgrid.defaults.guiStyle = 'bootstrap';
$.jgrid.cellattr = $.jgrid.cellattr || {};
$.extend($.jgrid.cellattr, {
    addDataAttr: function addDataAttr(rowId, cellVal, rawObject, cm, rdata) {
...
function jqBuildGrid() {
    ...
    return function (result) {
        ...
        Grid.obj.jqGrid({
            ...
            onSelectRow: function onSelectRow(id, isSelected) {
                if (switchCbColumn()) return;

                updateIdsOfSelectedRows(id, isSelected);
                Grid.subjectCountHtmlObj.html(Grid.subjectIdsObj.data('ids').length);
            },
            onSelectAll: function onSelectAll(rowIds, isSelected) {
                if (switchCbColumn()) return;
        ...
        {}, // add options
        {
            onclickSubmit: function onclickSubmit(jqXHR) {
                var $this = $(this),
        ...
        }, // delete options
        {
            width: 600,
            multipleSearch: true,
            recreateFilter: true // search options - define multiple search
        }).navButtonAdd('#pager', {
            caption: '',
...
function mapFormatter(column) {
    var functionsMapping = {
        "imagePreview": function imagePreview(cellValue, opts, rowObjects) {
            var url = encodeURIComponent(cellValue);
            return '<a href="' + cellValue + '" target="_new">View Image</a>&nbsp;&nbsp;' + '<a href="/images/preview?url=' + url + '" class="thumb-view">View Thumb</a>&nbsp;&nbsp;' + '<a href="' + cellValue + '" class="url-view">View Url</a>';
        }
    };

    for (var i = 0; i < column.length; i++) {
        var col = column[i];
        if (col.hasOwnProperty("formatter") && functionsMapping.hasOwnProperty(col.formatter)) {
...
exports.Manager = __webpack_require__(55);
...
Socket.transports = __webpack_require__(56);
...
/* WEBPACK VAR INJECTION */}.call(exports, __webpack_require__(120)(module), __webpack_require__(2)))
...
var update = __webpack_require__(20)("1a876232", content, false);
...
if(content.locals) module.exports = content.locals;
// add the styles to the DOM
var update = __webpack_require__(20)("66965fa0", content, false);
// Hot Module Replacement
if(false) {
 // When the styles change, update the <style> tags
 if(!content.locals) {
...
if(content.locals) module.exports = content.locals;
// add the styles to the DOM
var update = __webpack_require__(20)("540c65c3", content, false);
// Hot Module Replacement
if(false) {
 // When the styles change, update the <style> tags
 if(!content.locals) {
