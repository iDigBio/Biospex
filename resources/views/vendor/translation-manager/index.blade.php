<div style="width: 80%; margin: auto;">
    ...
    <p>
    @if(!isset($group))
        <form class="form-inline form-import" method="POST" action="{{ route('admin.translations.import') }}"
        ...
        <form class="form-inline form-find" method="POST" action="{{ route('admin.translations.find') }}"
              data-remote="true" role="form"
    ...
    @if(isset($group))
        <form class="form-inline form-publish" method="POST"
              action="{{ route('admin.translations.publish', [$group]) }}" data-remote="true" role="form"
            ...
            <div class="form-group">
                <select name="group" id="group" class="form-control group-select">
                    @foreach($groups as $key => $value)
                        <option data-route="{{ $key === '' ? route('admin.translations.index') : route('admin.translations.view') }}"
                                value="{{  $key }}"{{  $key == $group ? ' selected':'' }}>{{  $value }}</option>
                    @endforeach
                </select>
            </div>
        </form>
