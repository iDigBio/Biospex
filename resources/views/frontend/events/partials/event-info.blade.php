            ...
            <div class="row">
                ...
                @foreach($event->teams as $team)
                    <button type="button" class="btn btn-default btn-copy js-tooltip js-copy"
                            ...
                            data-copy="{{ route('events.get.join', [$team->uuid]) }}" title="Copy to clipboard">
                        <span class="fa fa-clipboard fa-lg"></span> {{ $team->title }}
                    </button>
                @endforeach
            </div>
            <div class="row top20">@lang('pages.project_url')
                : {!! link_to_route('projects.get.slug', $event->project->title, [$event->project->slug]) !!}
            </div>
        </div>
    </div>
</div>
        ...
        <div class="row">
            <div class="col-md-12">
                <button title="@lang('pages.downloadTitleUsers')" class="btn btn-success btn-sm"
                        type="button"
                        onClick="location.href='{{ route('admin.events.exportUsers', [$event->id]) }}'">
                ...
                <button title="@lang('pages.downloadTitleTranscriptions')" class="btn btn-success btn-sm"
                        type="button"
                        onClick="location.href='{{ route('admin.events.exportTranscriptions', [$event->id]) }}'">
