@foreach ($groups as $group)
    @foreach ($group->projects as $project)
        <tr id="test{{ $project->id }}">
            <td><a href="{{ route('admin.projects.show', [$project->id]) }}">{{ $project->title }}</a>
            </td>
            <td><a href="{{ route('admin.groups.show', [$group->id]) }}">{{ $group->title }}</a></td>
            <td class="fit buttons-sm">
                <button title="@lang('pages.viewTitle')" class="btn btn-primary btn-xs"
                        type="button"
                        onClick="location.href='{{ route('admin.projects.show', [$project->id]) }}'"><span
                            class="fa fa-eye fa-lg"></span> @lang('pages.view')</button>
                <button title="@lang('pages.dataTitle')" class="btn btn-inverse btn-xs"
                        type="button"
                        onClick="location.href='{{ route('admin.imports.import', [$project->id]) }}'">
                ...
                <button title="@lang('pages.duplicateTitle')" class="btn btn-success btn-xs"
                        type="button"
                        onClick="location.href='{{ route('admin.projects.duplicate', [$project->id]) }}'">
