    ...
    <div class="row">
        <div class="col-md-12">
                ...
                <div class="panel-body">
                    <div class="row">
                        ...
                        @if ( ! $expedition->downloads->isEmpty())
                            <button title="@lang('buttons.downloadTitle')" class="btn btn-success btn-sm" type="button" onClick="location.href='{{ route('web.downloads.index', [$expedition->project->id, $expedition->id]) }}'"><span class="fa fa-download fa-lrg"></span> @lang('buttons.download') </button>
                        @endif
                        ...
                        <div class="col-md-2">
                            <button title="@lang('buttons.ocrTitle')" class="btn btn-success btn-sm" type="button" onClick="location.href='{{ route('web.expeditions.ocr', [$expedition->project->id, $expedition->id]) }}'"><span class="fa fa-repeat fa-lrg"></span> @lang('buttons.ocr')</button>
                        </div>
                        <div class="col-md-2">
                        @if (is_null($expedition->workflowManager) || $expedition->workflowManager->stopped == 1)
                            <button title="@lang('buttons.processTitle')" class="btn btn-success btn-sm" onClick="location.href='{{ route('web.expeditions.process', [$expedition->project->id, $expedition->id]) }}'"><span class="fa fa-play fa-lrg"></span> @lang('buttons.process')</button>
                        @else
                            <button title="@lang('buttons.stopTitle')" class="btn btn-default btn-sm btn-danger action_confirm" href="{{ route('web.expeditions.stop', [$expedition->project->id, $expedition->id]) }}" data-token="{{ Session::getToken() }}" data-method="delete"><span class="fa fa-stop fa-lrg"></span> @lang('buttons.stop')</button></td>
                        @endif
                        </div>
                    </div>
                </div>
            ...
            <div class="table-responsive" id="jqtable">
                <input type="hidden" id="url" value="{{ URL::route('web.grids.show', [$expedition->project->id, $expedition->id]) }}">
